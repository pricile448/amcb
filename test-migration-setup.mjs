...
/**
 * Script de Test de la Configuration de Migration (ES Module)
 * 
 * Ce script teste la configuration avant de lancer la migration complète
 * Usage: node test-migration-setup.mjs
 */
...
let config;
try {
  config = await import('./migration-config.mjs');
  console.log('✅ Configuration chargée depuis migration-config.mjs');
} catch (error) {
...
function checkFirebaseConfig() {
  ...
  requiredFields.forEach(field => {
    if (!config.default.firebase[field] || config.default.firebase[field].includes('your-')) {
  ...
  if (missingFields.length > 0) {
    ...
    console.log('   Modifiez migration-config.mjs avec vos vraies valeurs Firebase');
    return false;
  }
...
async function testFirebaseConnection() {
  ...
  try {
    const app = initializeApp(config.default.firebase);
...
async function runTests() {
  ...
  if (allTestsPassed) {
    ...
    console.log('   2. Lancez un test de migration: node migrate-existing-users.mjs test USER_ID');
    console.log('   3. Lancez la migration complète: node migrate-existing-users.mjs');
  } else {
    ...
    console.log('   3. Relancez ce test: node test-migration-setup.mjs');
  }
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(`
Usage: node test-migration-setup.mjs [userId]
...
Exemples:
  node test-migration-setup.mjs                    # Test complet
  node test-migration-setup.mjs abc123             # Test avec un utilisateur spécifique
  node test-migration-setup.mjs --help             # Afficher cette aide
