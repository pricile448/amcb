/**
 * Script amélioré pour mettre à jour KycPage.tsx en remplaçant les textes codés en dur par des clés de traduction
 */
    ...
    const replacements = [
      {
        ...
        to: "const userName = user.displayName || user.firstName + ' ' + user.lastName || t('common.defaultUser');"
      },
      {
        from: /showError\('Erreur', 'Informations utilisateur manquantes'\);/g,
        to: "showError(t('common.error'), t('kyc.errors.userMissing'));"
      },
      {
      ...
      },
      {
        ...
        to: "showSuccess(t('common.success'), t('kyc.success.message'));"
      },
      {
        from: /showError\('Erreur', 'Une erreur est survenue lors de la soumission'\);/g,
        to: "showError(t('common.error'), t('kyc.errors.submissionFailed'));"
      }
    ];
