 ...
 * 
 * Usage: node migrate-existing-users.cjs
 */
...
if (command === 'test' && args[1]) {
  ...
  console.log(`
Usage: node migrate-existing-users.cjs [command]
...
Exemples:
  node migrate-existing-users.cjs           # Migration complète
  node migrate-existing-users.cjs test abc123  # Test sur un utilisateur
  node migrate-existing-users.cjs help     # Aide
  `);
  process.exit(0);
} else {
  // Migration complète
