...
const Logo: React.FC<LogoProps> = ({ 
  ...
  // Gestion des erreurs de logo - Log uniquement
  const handleLogoError = (e: React.SyntheticEvent<HTMLImageElement, Event>, variantType: string) => {
    console.error(`Erreur de chargement du logo Cloudinary (${variantType}):`, e);
    // Pas de fallback local - on garde l'image cassée pour que l'utilisateur voie le problème
  };
  ...
  if (variant === 'icon') {
    return (
      <div className={`flex items-center ${className}`}>
        <div className={`${cardStyles} ${cardPadding}`}>
          <img 
            src={logoService.getIconLogoUrl()}
            alt="AmCbunq" 
            className={`${sizeClass} w-auto transition-all duration-300 ${animationClass} ${responsiveClass}`}
            onError={(e) => handleLogoError(e, 'icon')}
          />
        </div>
      </div>
    );
  ...
  return (
    <div className={`flex flex-col items-center ${className}`}>
      <div className={`flex items-center ${cardStyles} ${cardPadding}`}>
        <img 
          src={logoService.getMainLogoUrl()}
          ...
          onError={(e) => handleLogoError(e, 'full')}
        />
      </div>
      {showTagline && (
