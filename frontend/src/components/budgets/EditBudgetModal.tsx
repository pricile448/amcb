...
const EditBudgetModal: React.FC<EditBudgetModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.amount > 999999) {
      newErrors.amount = 'Le montant ne peut pas dépasser 999 999 €';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      logger.debug('Début de la modification du budget...');
      ...
      const updatedBudgetData = {
        ...
        spent: budget.spent, // Garder le montant dépensé actuel
        status: budget.status, // Garder le statut actuel
        updatedAt: new Date()
      };

      logger.debug('Données du budget à modifier:', updatedBudgetData);
      ...
      logger.success('Budget modifié avec succès:', updatedBudgetData);
